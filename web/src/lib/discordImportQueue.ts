...
function nowTimestamp(now: Date): string {
  ...
  if (!Number.isFinite(milliseconds) || milliseconds < MIN_DISCORD_DATE || milliseconds > MAX_SAFE_DATE) {
    validationError('now', 'invalid date')
  }
...
export function parseDiscordImportQueue(input: unknown): DiscordImportQueue {
  return parseQueue(input, true)
}

export function createDiscordImportQueue(
  manifestInput: unknown,
  now: Date = new Date(),
): DiscordImportQueue {
  const manifest = parseDiscordImportManifest(manifestInput)
  return {
    version: DISCORD_IMPORT_MANIFEST_VERSION,
    manifest,
    items: manifest.cards.map(createQueueItem),
    importedHashes: [],
    updatedAt: nowTimestamp(now),
  }
}

export function mergeDiscordImportQueue(
  ...
  if (previousInput === null || previousInput === undefined) {
    return createDiscordImportQueue(manifest, now)
  }

  const previous = parseQueue(previousInput, true)
  ...
  return {
    ...
    manifest,
    items,
    importedHashes: [...previous.importedHashes],
    updatedAt: nowTimestamp(now),
  }
}

export function updateDiscordImportQueueItem(
  queueInput: unknown,
  id: string,
  patch: DiscordImportQueueItemPatch,
  now: Date = new Date(),
): DiscordImportQueue {
  const queue = parseQueue(queueInput, false)
  const targetId = asSnowflake(id, 'id')
  const itemIndex = queue.items.findIndex((item) => item.id === targetId)
  ...
  if (patch.selected !== undefined && typeof patch.selected !== 'boolean') {
    validationError('patch.selected', 'expected a boolean')
  }
  const error = patch.error === undefined
    ? current.error
    : asString(patch.error, 'patch.error', { max: 1_000, allowEmpty: true })
  const importedAvatar = patch.importedAvatar === undefined
    ? current.importedAvatar
    : asSafeFileName(patch.importedAvatar, 'patch.importedAvatar')

  const nextItem: DiscordImportQueueItem = {
    id: targetId,
    status,
  ...
  let importedHashes = queue.importedHashes
  if (patch.importedHash !== undefined) {
    const hash = asString(patch.importedHash, 'patch.importedHash', { max: 128 })
    if (!HASH_PATTERN.test(hash)) validationError('patch.importedHash', 'invalid hash')
    if (!importedHashes.includes(hash)) {
      if (importedHashes.length >= MAX_IMPORTED_HASHES) {
        validationError('queue.importedHashes', `array exceeds ${MAX_IMPORTED_HASHES} items`)
      }
      importedHashes = [...importedHashes, hash]
    }
  }
...
export function requestDiscordImportBatch(
  queueInput: unknown,
  now: Date = new Date(),
): DiscordImportQueue {
  const queue = parseQueue(queueInput, false)
  const cardById = new Map(queue.manifest.cards.map((card) => [card.id, card]))
  const cardIds = queue.items
    .filter((item) => {
      const card = cardById.get(item.id)
  ...
  if (!cardIds.length) validationError('queue.items', 'no selected cards are ready to import')
...
export function cancelDiscordImportBatch(
  queueInput: unknown,
  now: Date = new Date(),
): DiscordImportQueue {
  const queue = parseQueue(queueInput, false)
  const rest = { ...queue }
  delete rest.importRequest
  return {
    ...rest,
    updatedAt: nowTimestamp(now),
  }
...
export function completeDiscordImportBatchItem(
  queueInput: unknown,
  id: string,
  now: Date = new Date(),
): DiscordImportQueue {
  const queue = parseQueue(queueInput, false)
  const targetId = asSnowflake(id, 'id')
  if (!queue.importRequest?.cardIds.includes(targetId)) return queue

  const importRequest = queue.importRequest
  const remainingCardIds = importRequest.cardIds.filter((cardId) => cardId !== targetId)
  const rest = { ...queue }
  delete rest.importRequest
  return {
    ...rest,
    ...(remainingCardIds.length
      ? { importRequest: { ...importRequest, cardIds: remainingCardIds } }
      : {}),
    updatedAt: nowTimestamp(now),
  }
}

export function resolveDiscordImportBatchItem(
  queueInput: unknown,
  id: string,
  patch: DiscordImportQueueItemPatch,
  now: Date = new Date(),
): DiscordImportQueue {
  const updated = updateDiscordImportQueueItem(queueInput, id, patch, now)
  return completeDiscordImportBatchItem(updated, id, now)
}
...
export function saveDiscordImportQueue(
  queueInput: unknown,
  handle: string,
  storage?: Storage | null,
): boolean {
  ...
  try {
    const queue = parseQueue(queueInput, false)
    target.setItem(getDiscordImportQueueStorageKey(handle), JSON.stringify(queue))
    return true
  } catch {
    return false
  }
...
export function clearDiscordImportQueue(handle: string, storage?: Storage | null): boolean {
  const target = resolveStorage(storage)
  if (!target) return false

  try {
    target.removeItem(getDiscordImportQueueStorageKey(handle))
    return true
  } catch {
    return false
  }
