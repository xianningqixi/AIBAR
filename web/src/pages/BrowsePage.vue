...
const charFilterOptions = [
  ...
  { value: 'recent' as const, label: '最近' },
            ...
            <section v-if="!loading && activeTab === 'characters'">
              ...
              >
                <button
                  ...
                  class="shrink-0 rounded-full bg-brand-500/10 px-3.5 py-1.5 text-sm font-medium text-brand-300 ring-1 ring-brand-500/40 transition-all"
                ...
                <button
                  ...
                  :key="t.tag"
                  :class="[
                    'shrink-0 rounded-full px-3.5 py-1.5 text-sm font-medium ring-1 transition-all',
                ...
                >
                  #{{ t.tag }} <span class="ml-0.5 text-ink-muted">{{ t.count }}</span>
                </button>
              </div>
            ...
            <section v-if="!loading && activeTab === 'chats'">
              ...
              <div v-else class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                <button
                  ...
                  :key="entry.avatar ? `${entry.avatar}:${entry.file_name}` : entry.file_name"
                  class="flex h-full flex-col rounded-2xl bg-surface p-5 text-left shadow-sm ring-1 ring-border-subtle transition-all duration-200 hover:-translate-y-0.5 hover:ring-brand-500/40 hover:shadow-glow focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-500"
                  @click="openChatEntry(entry)"
                >
                  <div class="flex gap-4">
                    <img
                    ...
                    <div class="min-w-0 flex-1">
                      <h4 class="truncate text-base font-semibold text-ink-primary">{{ getChatTitle(entry) }}</h4>
                      <p class="mt-1 truncate text-sm text-ink-muted">{{ getChatCharacterName(entry) }}</p>
