...
const ledgerTotalPages = computed(() => Math.max(1, Math.ceil(billing.ledger.length / LEDGER_PAGE_SIZE)))
const paginatedLedger = computed(() => {
  const start = (ledgerPage.value - 1) * LEDGER_PAGE_SIZE
  return billing.ledger.slice(start, start + LEDGER_PAGE_SIZE)
})
...
const { loading: redeeming, run: redeem } = useAsyncAction(async () => {
...
<template>
  <div class="min-h-[100dvh] bg-bg">
    ...
    <main class="mx-auto max-w-4xl space-y-6 px-5 py-6 md:px-8 lg:px-10">
      <AppCard padding="md" class="flex items-center gap-4">
        <div class="group relative">
          ...
          <div class="absolute inset-0 flex cursor-pointer items-center justify-center rounded-xl bg-black/40 text-[10px] font-medium text-white opacity-0 transition-opacity group-hover:opacity-100" @click="ui.addToast('头像更换功能即将上线', 'info')">
            更换
          </div>
        </div>
        <div class="min-w-0">
          <h1 class="truncate text-xl font-semibold text-ink-primary">{{ session.user?.name }}</h1>
          <p class="mt-1 text-sm text-ink-muted">@{{ session.user?.handle }} · {{ session.isAdmin ? '管理员' : '普通用户' }}</p>
        </div>
      </AppCard>

      <AppCard v-if="session.user && !session.user.password" padding="md" tone="warning"
        title="安全提醒"
        description="当前账号没有密码。开放给其他人访问前，请先设置密码。"
      />

      <AppCard padding="md" class="space-y-6">
        ...
        <form class="mt-6 flex max-w-2xl flex-col gap-3 sm:flex-row sm:items-center" @submit.prevent="redeem">
          ...
          <AppButton type="submit" class="w-full sm:w-auto" :loading="redeeming">兑换积分</AppButton>
        </form>
        <div class="mt-6">
          <div class="mb-3 flex items-center justify-between">
            <h2 class="text-sm font-semibold text-ink-primary">最近积分明细</h2>
            <span class="text-xs text-ink-muted">共 {{ billing.ledger.length }} 条</span>
          </div>
          <div v-if="billing.ledger.length" class="divide-y divide-border-subtle border-y border-border-subtle">
            <div v-for="entry in paginatedLedger" :key="entry.id" class="flex items-center gap-4 py-3 text-sm">
              ...
              <span class="shrink-0 font-semibold tabular-nums" :class="entry.delta >= 0 ? 'text-success' : 'text-ink-primary'">
                {{ entry.delta >= 0 ? '+' : '' }}{{ formatPoints(entry.delta) }}
              </span>
            </div>
          </div>
          <p v-else class="py-5 text-sm text-ink-muted">暂无积分变动。</p>
          <div v-if="ledgerTotalPages > 1" class="mt-3 flex items-center justify-end gap-2">
            <AppButton size="sm" variant="secondary" :disabled="ledgerPage <= 1" @click="ledgerPage--">上一页</AppButton>
            <span class="text-xs text-ink-muted">{{ ledgerPage }} / {{ ledgerTotalPages }}</span>
            <AppButton size="sm" variant="secondary" :disabled="ledgerPage >= ledgerTotalPages" @click="ledgerPage++">下一页</AppButton>
          </div>
        </div>
      </AppCard>
        ...
        <form class="max-w-2xl space-y-4" @submit.prevent="savePassword">
          ...
          <div class="grid gap-4 sm:grid-cols-2">
            <AppFormField label="新密码" required>
              <AppInput v-model="password.next" type="password" autocomplete="new-password" />
            </AppFormField>
            <AppFormField label="确认新密码" required>
              <AppInput v-model="password.confirm" type="password" autocomplete="new-password" />
            </AppFormField>
          </div>
          <div class="flex items-center justify-end gap-3">
            <AppButton type="submit" :loading="saving">更新密码</AppButton>
          </div>
        </form>
      </AppCard>

      <AppCard padding="md" class="flex flex-wrap items-center justify-end gap-3">
