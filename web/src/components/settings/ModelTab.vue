  ...
  <div class="grid gap-4 lg:grid-cols-[300px_1fr]">
      ...
      <div class="space-y-1">
        ...
        >
          ...
          <div class="flex items-center justify-between gap-2">
            ...
            <span v-if="models.activeProfileId === profile.id" class="text-[11px] text-success shrink-0">默认</span>
          </div>
        ...
        <AppEmpty v-if="!models.profiles.length" icon="box" title="暂无模型配置" description="点击上方新建或写入示例。">
          <template #actions>
            <AppButton size="sm" @click="addProfile()">新建 Profile</AppButton>
          </template>
        </AppEmpty>
      </div>
    </AppCard>
    ...
    <div class="space-y-4">
      <AppCard padding="md" tone="glow" class="space-y-5">
        ...
        <div class="space-y-4">
          <h3 class="flex items-center gap-2 text-sm font-semibold text-ink-primary">
            <span class="h-4 w-1 shrink-0 rounded-full bg-brand-gradient" />基础
          </h3>
          <div class="grid gap-4 sm:grid-cols-2">
            ...
            <AppFormField class="flex items-end">
              <AppCheckbox v-model="setupDraft.enabled">对用户启用</AppCheckbox>
            </AppFormField>
          </div>
        </div>
        ...
        <div class="space-y-4 border-t border-border-subtle pt-4">
          <h3 class="flex items-center gap-2 text-sm font-semibold text-ink-primary">
            <span class="h-4 w-1 shrink-0 rounded-full bg-brand-gradient" />连接
          </h3>
          <div class="grid gap-4 sm:grid-cols-2">
            <AppFormField
        ...
        <div class="space-y-4 border-t border-border-subtle pt-4">
          <h3 class="flex items-center gap-2 text-sm font-semibold text-ink-primary">
            <span class="h-4 w-1 shrink-0 rounded-full bg-brand-gradient" />计价
          </h3>
          <div class="grid gap-4 sm:grid-cols-2">
        ...
        <div class="flex flex-col gap-3 border-t border-border-subtle pt-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <AppButton variant="gradient" :loading="setupDraft.testing" @click="saveSetupProfile">
            保存并测试
          </AppButton>
        </div>
      </AppCard>
      ...
      <AppCard v-if="selectedProfile" collapsible title="高级 Profile 管理" :default-open="false">
        ...
        >
          <span class="w-1.5 h-1.5 rounded-full" :class="testResults[selectedProfile.id].ok ? 'bg-success' : 'bg-danger'" />
          <span class="flex-1">
        ...
        <div class="space-y-4">
          <div class="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center sm:justify-between">
            ...
            <div class="flex flex-wrap items-center gap-3">
              <AppCheckbox
                :model-value="selectedProfile.enabled !== false"
                @update:model-value="models.updateProfile(selectedProfile.id, { enabled: $event })"
              >启用</AppCheckbox>
              <span
                :class="[
                  'text-[11px] inline-flex items-center gap-1.5',
                  models.hasSavedApiKey(selectedProfile) || selectedProfile.source === 'custom' ? 'text-success' : 'text-warning',
                ]"
              >
                <span class="w-1.5 h-1.5 rounded-full" :class="models.hasSavedApiKey(selectedProfile) || selectedProfile.source === 'custom' ? 'bg-success' : 'bg-warning'" />
                {{ selectedProfile.source === 'custom' ? '本地/兼容可无 Key' : models.getProviderSecretLabel(selectedProfile) }}
              </span>
              <AppButton
                size="sm"
                variant="secondary"
                :loading="testing[selectedProfile.id]"
          ...
          <div class="grid gap-4 md:grid-cols-5 pt-1">
            <div>
              ...
              <AppSlider
                :model-value="selectedProfile.temperature"
                :min="0"
                :max="2"
                :step="0.1"
                @update:model-value="(v) => models.updateProfile(selectedProfile!.id, { temperature: v })"
              />
            </div>
            <div>
              <label class="block text-xs font-medium text-ink-secondary mb-1.5">
              ...
              <AppSlider
                :model-value="selectedProfile.topP"
                :min="0"
                :max="1"
                :step="0.05"
                @update:model-value="(v) => models.updateProfile(selectedProfile!.id, { topP: v })"
              />
            </div>
