...
defineProps<{
  title?: string
  description?: string
  icon?: 'chat' | 'box' | 'search' | 'book'
}>()
</script>

<template>
  <div class="flex flex-col items-center justify-center py-16 px-6 text-center">
    <div class="mb-4 flex h-16 w-16 items-center justify-center rounded-2xl bg-brand-soft ring-1 ring-brand-500/20 text-brand-300">
      <slot name="icon">
        <svg v-if="icon === 'chat'" class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <svg v-else-if="icon === 'search'" class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
        </svg>
        <svg v-else-if="icon === 'book'" class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
