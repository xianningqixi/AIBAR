...
The local dashboard owns the synchronized hot list, checkbox selection, publish request, and per-item outcomes. The AIBAR `/hub?source=discord` page exposes the manual Discord card public-publishing entry (PNG/JSON/CHARX/BYAF/YAML). A private character file is only server-side parsing staging; success requires a visible public community work or a server-hash duplicate link.
